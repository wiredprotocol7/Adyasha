...
def send_request(context):
  global request
  global baseurl
  request = urlresponse.api_call(method= '{method}',url='{baseurl}'+'/{endpoint}',load={parameter},headers={headers},auth={auth})
...
def createfile(typefile, base_file_name):  # create a file
    ...
    filename = os.getcwd() + "\\testcase_generator\\" + filename
    ...
    try:
        ...
        print(error)
        print("unable to create file in createfile function")
...
def create_test_script(headers, auth, base_file_name, method, endpoint, parameter=None,
    ...
    try:  # exception handeling
        ...
        with open(file.name, 'w') as f:  # open(file.name, w) is for writing things into the file
            ...
            f.write("    resp=urlresponse.api_call(method,endpoint,parameter,headers=headers,auth=auth)\n")
            f.write("    print(resp)\n")
            f.write(codebot)
    except Exception as error:
        print(error)
...
create_file_final(baseurl=baseurl, typescript='cucumber',
                  projectid='788-pet-7867877hhgjhgjjhgjhgjhghjbnbbvnb68768768768768768768768', userstoryid='storeorder',
                  featurename='Finding the order using order id using GET API', scenarioname='Get order by order id',
                  endpoint='store/order/2', method='GET', headers={'Content-type': 'application/json'})
...
# create_file(baseurl=baseurl3,typescript='addtostepsscript',projectid='peet',userstoryid='storee',featurename = 'testing github API authentication', scenarioname ='API authentication for github' ,endpoint =endpoint3 , pointingattribute = 'githubuserid2', method='GET',parameter={'type':'all'},parameter_values='githubuserid2', headers=githubheader)
create_file_final(baseurl=baseurl2, contenttype='xml', typescript='cucumber', projectid='http', userstoryid='authcheck',
                  featurename='testing httpbin API authentication', scenarioname='API authentication for httpbin',
                  endpoint=endpoint2, method='GET', parameter=None, auth="('user', 'pass')", auth_type='HTTPDigestAuth')
# create_file_new(baseurl=baseurl3,contenttype='json',typescript='addtostepsscript',projectid='peet',userstoryid='storee',featurename = 'testing github API authentication', scenarioname ='API authentication for github' ,endpoint =endpoint3 , pointingattribute = 'githubuserid2', method='GET',parameter={'type':'all'},parameter_values='githubuserid2',auth_type='headers')
create_file_final(baseurl=baseurl3, contenttype='json', typescript='cucumber', projectid='peet', userstoryid='storee',
                  featurename='testing github API authentication', scenarioname='API authentication for github',
                  endpoint=endpoint3, pointingattribute='githubuserid', method='GET', parameter=None,
                  auth="HTTPBasicAuth('adya1904' ,'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')")